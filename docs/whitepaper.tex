...
The framework engine (\texttt{mod/core/mod.py}) is composed from focused mixins---\texttt{FsMixin} (path \& file utilities), \texttt{DeployMixin} (server, Docker, deployment), and \texttt{FactoryMixin} (new, fork, clone, rm operations)---providing the following subsystems:
...
\begin{enumerate}
    ...
    \item \textbf{Serve}: \texttt{m serve name} auto-generates a FastAPI server for the module.
...
Select modules ship as complete applications with their own frontend, API, and smart contracts:

\begin{itemize}
    \item \textbf{BlocTime App}: Next.js + FastAPI staking dashboard. Users stake NativeToken, view positions, claim rewards. Ports 8851 (API) / 8852 (frontend).
    \item \textbf{EmbedCode}: Code embedding and execution platform. Next.js frontend with Tailwind CSS, FastAPI backend for code processing and deployment.
\end{itemize}
