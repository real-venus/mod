...
async function bridgeApi(fn: string, params: Record<string, any> = {}, token: string = '') {
  // Call bridge API endpoints directly
  const hasBody = Object.keys(params).length > 0 || ['commit', 'update_commitment', 'claim'].includes(fn)
  const method = hasBody ? 'POST' : 'POST'
  const headers: Record<string, string> = { 'Content-Type': 'application/json' }
  if (token) headers['token'] = token

  const res = await fetch(`${BRIDGE_API}/${fn}`, {
    method,
    headers,
...
function BridgePageInner() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [statsData, balancesData, claimsData, commitmentsData] = await Promise.all([
        bridgeApi('status').catch(() => null),
        bridgeApi('get_total_balances').catch(() => ({})),
        bridgeApi('get_claims').catch(() => ({})),
        bridgeApi('get_commitments').catch(() => ({})),
      ])
  ...
  const handleCommit = useCallback(async (sourceAddress: string, sourceType: SourceType) => {
    ...
    try {
      ...
      const result = await bridgeApi('commit', {
  ...
  const handleUpdate = useCallback(async (sourceAddress: string, sourceType: SourceType, newEvmAddress: string) => {
    ...
    try {
      ...
      const result = await bridgeApi('update_commitment', {
