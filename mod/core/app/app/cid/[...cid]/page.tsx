...
export default function CidViewer() {
  ...
  const cidPath = Array.isArray(params.cid) ? params.cid.join('/') : (params.cid as string);
  ...
  useEffect(() => {
    if (!cidPath || !client) return;

    const fetchContent = async () => {
      setLoading(true);
      setError(null);
      try {
        const res = await client.call('get', { cid: cidPath });
        setContent(res);
        ...
        <div className={`border ${isLight ? 'border-black/10' : 'border-white/10'} rounded-lg p-6`}>
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <div className={`text-[10px] font-mono mt-1 ${isLight ? 'text-black/40' : 'text-white/40'}`}>
                {cidPath.startsWith('Qm') || cidPath.includes('/Qm') ? 'CIDv0' :
                 cidPath.includes('bafy') ? 'CIDv1 (dag-pb)' :
                 cidPath.includes('bafk') ? 'CIDv1 (raw)' :
                 cidPath.includes('bafyr') ? 'CIDv1 (dag-cbor)' :
                 'CID'}
              </div>
            </div>
            <CopyButton text={cidPath} size="md" />
          </div>
        </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => {
                    ...
                    a.href = url;
                    a.download = `${cidPath}.${contentType === 'json' ? 'json' : 'txt'}`;
                    a.click();
