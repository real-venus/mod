...
export function TransactionCard({ tx, idx }: TransactionCardProps) {
  ...
  const renderValue = (value: any) => {
    if (value === null || value === undefined) {
      return <span className="text-gray-600 font-mono text-sm">null</span>
    }
          ...
          {Object.entries(value).map(([k, v]) => (
            <div key={k} className="bg-black/40 p-2 rounded-lg border-2 border-gray-800">
              <div className="flex items-center gap-2">
                <span className="text-cyan-400 font-mono text-sm font-semibold">{k}:</span>
                <span className="text-gray-300 break-all font-mono text-sm">{typeof v === 'object' ? JSON.stringify(v) : String(v)}</span>
    ...
    return (
      <div className="flex items-center gap-2">
        <span className="text-gray-300 break-all font-mono text-sm">{typeof value === 'object' ? JSON.stringify(value) : String(value)}</span>
    ...
    >
      <div className="p-4">
        <div className="flex items-center gap-2 flex-wrap">
          <CubeIcon className="w-6 h-6" style={{ color: txColor }} />
          <div className="flex items-center gap-1 bg-black/40 border-2 rounded-lg px-2 py-1" style={{ borderColor: `${txColor}40` }}>
            <code className="text-sm font-mono" style={{ color: txColor }}>{tx.fn}</code>
            <CopyButton text={tx.fn} size="sm" />
          </div>
          <span className={`${statusColors.text} font-bold text-sm px-2 py-1 bg-black/40 rounded-lg border-2 ${statusColors.border}`}>
            {getStatusEmoji(tx.status)}
          </span>
          <span className="bg-black/40 border-2 border-green-900/50 px-2 py-1 rounded-lg font-mono text-green-400 text-sm font-bold">
            ${costUsd.toFixed(4)}
          </span>
          {tx.cid && (
            <div className="flex items-center gap-1 bg-black/40 border-2 border-green-500/30 rounded-lg px-2 py-1">
              <code className="text-xs font-mono" style={{ color: '#10b981' }}>{shorten(tx.cid)}</code>
              <CopyButton text={tx.cid} size="sm" />
            </div>
          )}
          <div className="flex items-center gap-1 bg-black/40 border-2 border-blue-500/30 rounded-lg px-2 py-1">
        ...
        {isExpanded && (
          <div className="mt-3 space-y-2 pt-3 border-t-2 border-gray-800">
            <div className="flex items-center gap-2 flex-wrap">
              {tx.signature && (
                <span className="bg-black/40 border-2 border-orange-900/50 rounded-lg px-2 py-1 flex items-center gap-1 text-xs">
              ...
              {tx.key && (
                <span className="bg-black/40 border-2 border-purple-900/50 rounded-lg px-2 py-1 flex items-center gap-1 text-xs">
              ...
              {tx.delta !== undefined && (
                <span className="bg-black/40 border-2 border-cyan-900/50 px-2 py-1 rounded-lg font-mono text-cyan-400 text-xs font-semibold">
            ...
            {tx.params && (
              <div className="bg-black/40 border-2 border-gray-800 rounded-lg overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between p-2 hover:bg-gray-900/50 transition-all"
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-gray-500 text-xs">{isParamsExpanded ? '▼' : '▶'}</span>
                  </div>
                </button>
                {isParamsExpanded && (
                  <div className="p-2 bg-black/60 border-t-2 border-gray-800">
              ...
              <div className="bg-black/40 border-2 border-gray-800 rounded-lg overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-gray-500 text-xs">{isResultsExpanded ? '▼' : '▶'}</span>
                  </div>
                </button>
                {isResultsExpanded && (
                  <div className="p-2 bg-black/60 border-t-2 border-gray-800">
            ...
            {tx.client && (
              <div className="bg-black/40 border-2 border-gray-800 rounded-lg p-2">
                <div className="flex items-center gap-2">
