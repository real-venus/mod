...
interface ModApiWithOwnerSelectorProps {
  mod: ModuleType
  allowOwnerSelection?: boolean
}

export default function ModApiWithOwnerSelector({ mod, allowOwnerSelection = true }: ModApiWithOwnerSelectorProps) {
  ...
  const [owner, setOwner] = useState<string>(mod.owner || mod.key || '')
  ...
  useEffect(() => {
    if (mod.owner || mod.key) {
      setOwner(mod.owner || mod.key || '')
    }
  }, [mod.owner, mod.key])
  ...
  const handleExecute = async () => {
    ...
    setError(null)
    try {
      // Use the selected owner when calling the module
      const res = await client.call(selectedFunction, { 
        ...params, 
        mod: mod.name, 
        key: owner // Use selected owner instead of mod.key
      })
      setResult(res)
    } catch (err: any) {
