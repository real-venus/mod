...
export const UserCard = ({ user, mode = 'explore' }: UserCardProps) => {
  ...
  const [isModsHovered, setIsModsHovered] = useState(false)
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex items-center gap-2 bg-gradient-to-r from-black/50 to-black/30 rounded-lg px-4 py-2 flex-1 shadow-lg">
                ...
                >
                  ...
                  {isQrHovered && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 p-2 bg-black/95 rounded-lg border-2 z-[9999] shadow-2xl" style={{ borderColor: userColor }}>
                      <QRCode value={websiteUrl} size={100} color={userColor} />
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {user.mods && user.mods.length > 0 && (
                <div
                  className="flex items-center gap-1.5 bg-gradient-to-r from-purple-900/30 to-pink-900/30 rounded-lg px-2 py-1.5 shadow-md transition-all hover:scale-105"
                  onMouseEnter={() => setIsModsHovered(true)}
                  onMouseLeave={() => setIsModsHovered(false)}
                >
              ...
              {user.balance !== undefined && (
                <div
                  className="flex items-center gap-1.5 bg-gradient-to-r from-emerald-900/30 to-teal-900/30 rounded-lg px-2 py-1.5 shadow-md transition-all hover:scale-105"
