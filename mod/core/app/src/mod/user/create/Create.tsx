...
export const Create: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (walletMode === 'subwallet' && address) {
  ...
  useEffect(() => {
    const fetchMyMods = async () => {
      ...
      try {
        ...
        const userMods = response.filter((mod: ModuleType) => mod.key === user.key)
        setMyMods(userMods)
      } catch (err) {
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="space-y-5 p-6 rounded-xl bg-gradient-to-br from-blue-500/10 via-purple-500/10 to-pink-500/10 border-2 border-blue-500/30 shadow-2xl">
        <div className="flex items-center gap-3 pb-4 border-b-2 border-blue-500/30">
          ...
          <h3 className="text-2xl font-black text-blue-400 font-mono uppercase tracking-wide">
            {mode === 'register' ? 'Register Module' : 'Update Module'}
          </h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <input
              ...
              disabled={isLoading || (mode === 'update' && selectedMod !== 'new')}
        ...
        >
          {isLoading ? (
            ...
            <>
              {mode === 'register' ? <Plus size={20} /> : <Save size={20} />}
              <span>{mode === 'register' ? 'REGISTER MODULE' : 'UPDATE MODULE'}</span>
            </>
          )}
        </button>
      </div>
