...
sr25519_key = m.key(crypto_type='sr25519')  # Default
ed25519_key = m.key(crypto_type='ed25519')
ecdsa_key = m.key(crypto_type='ecdsa')  # Compatible with Ethereum

# Create key from mnemonic
mnemonic = "bottom drive obey lake curtain smoke basket hold race lonely fit walk"
key_from_mnemonic = m.key(mnemonic=mnemonic)

# Create key from private key
key_from_private = m.key(private_key="0x...")  # Hex string or bytes
```
...
key_from_json = m.key().from_json(key_json)
```
