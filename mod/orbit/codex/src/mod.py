...
class Mod:
    ...
    def _request(self, method: str, path: str, data: dict = None, timeout: int = 30) -> dict:
        ...
        try:
            ...
            raise ConnectionError(
                ...
                f"start it with: cd mod/orbit/codex/api && cargo run\n{e}"
            )
    ...
    def _start_api(self) -> bool:
        ...
        binary = os.path.join(api_dir, 'target', 'release', 'codex-jobs')
        ...
        def _monitor():
            while self._api_managed:
                ...
                if idle >= self._idle_timeout:
                    print(f"[codex] API idle for {int(idle)}s — shutting down")
                    self._stop_api()
                    return
    ...
    def _ensure_api(self):
        ...
        if self._server_available():
            return True
        print("[codex] API not running — starting automatically...")
        if self._start_api():
            print(f"[codex] API started on port {self._api_port()} (idle timeout: {self._idle_timeout}s)")
            return True
        raise ConnectionError(
    ...
    def kill(self, pid: int = None, port: int = None, sig: str = "SIGKILL") -> dict:
        """Kill a process by PID, port, or both codex services (API+App) if no args given. Owner-only."""
    ...
    def models(self, search: str = 'codex') -> list:
    ...
    def install(self, host: str = None, key_path: str = None, **kwargs) -> dict:
        ...
        Usage:
            m codex/install              # local install + auth setup
            m codex/install user@myserver # remote install
        """
        if not host:
        ...
        if not ok:
            ...
            ok, out, err = self._ssh(
                host, self._ssh_source_nvm('npm install -g @openai/codex'),
                key_path=key_path, timeout=120,
            )
        if not ok:
    ...
    def _install_local(self, **kwargs) -> dict:
        ...
        local_bin = os.path.join(mod_dir, 'node_modules', '.bin', 'codex')
    ...
    def _install_rustup(self) -> Optional[str]:
        ...
        if os.path.exists(cargo_bin):
            return cargo_bin
        print('[codex] cargo missing — installing rustup (this may take a minute)')
        try:
            r = subprocess.run(
            ...
            print(f'[codex] rustup install failed: {e}')
            return None
        if r.returncode != 0 or not os.path.exists(cargo_bin):
            print(f'[codex] rustup install failed: {r.stderr[-200:]}')
            return None
        print('[codex] rustup installed')
        return cargo_bin
    ...
    def ensure_env(self, app_dir: str = None, api_dir: str = None):
        ...
        if (app_dir / 'package.json').exists():
            if not (app_dir / 'node_modules').is_dir():
                print('[codex] node_modules missing — running npm install')
                try:
                    r = subprocess.run(
                    ...
                    print('[codex] npm install failed: npm not found in PATH')
                else:
                    if r.returncode != 0:
                        results['app_install'] = {'ok': False, 'error': r.stderr[-500:]}
                        print(f'[codex] npm install failed: {r.stderr[-200:]}')
                    else:
                        results['app_install'] = {'ok': True}
                        print('[codex] npm install done')
            else:
                results['app_install'] = {'ok': True, 'cached': True}

        # ── API: Rust binary ──
        binary = api_dir / 'target' / 'release' / 'codex-jobs'
        if (api_dir / 'Cargo.toml').exists() and not binary.exists():
            ...
            if not cargo:
                ...
                print('[codex] API binary missing — running cargo build --release')
                try:
                    r = subprocess.run(
                    ...
                    print('[codex] cargo build failed: cargo not found in PATH')
                else:
                    if r.returncode != 0:
                        results['api_build'] = {'ok': False, 'error': r.stderr[-500:]}
                        print(f'[codex] cargo build failed: {r.stderr[-200:]}')
                    else:
                        results['api_build'] = {'ok': True}
                        print('[codex] cargo build done')
        else:
    ...
    def serve(self, api_port=None, app_port=None, dev=True):
        ...
        if 'api' in results:
            ...
            if not api_live:
                ...
                print(f'[codex] API failed to start on :{api_port}')
                if tail:
                    print(tail[-300:])
            else:
        ...
        if 'app' in results:
            ...
            if not app_live:
                ...
                print(f'[codex] App failed to start on :{app_port}')
                ...
                if not env_result.get('app_install', {}).get('cached'):
                    ...
                    print('[codex] retrying — reinstalling node_modules')
                    import shutil
                    ...
                    checks['app']['live'] = app_live
                    if app_live:
                        print(f'[codex] App recovered on :{app_port}')
                    else:
                        tail = self._tail_log(str(log_dir / 'app.log'))
                        checks['app']['error'] = tail
                        print(f'[codex] App still failing after retry')
            else:
                print(f'[codex] App live on :{app_port}')
        ...
        if all_live:
            try:
                ...
                print(f'[codex] CID registration skipped: {e}')
