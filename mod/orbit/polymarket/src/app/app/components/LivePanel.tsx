...
const CAPITAL_PRESETS = [10, 100, 1000, 5000, 10000];
    ...
    <div className="space-y-2">
      ...
      <div className="pixel-panel border-2 border-pixel-border">
        <div className="px-3 py-2 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-pixel-white tracking-wider">LIVE COPY</span>
            {isLive && (
          ...
          <div className="flex items-center gap-1.5">
            {isLive && status === "running" && (
              <button
                onClick={pauseLive}
                className="pixel-btn text-[7px] px-1.5 py-0.5 border-amber-400/60 text-amber-400 hover:bg-amber-400/10"
              >
                PAUSE
              </button>
            )}
            {isLive && status === "paused" && (
              <button
                onClick={resumeLive}
                className="pixel-btn text-[7px] px-1.5 py-0.5 border-green-400/60 text-green-400 hover:bg-green-400/10"
              >
                RESUME
              </button>
            )}
            <button
              ...
              className={`pixel-btn text-[8px] px-2.5 py-1 transition-colors ${
                isLive
        ...
        {confirmed && !isLive && (
          <div className="px-3 py-2 border-t border-red-400/30 bg-red-400/5">
            <p className="text-[8px] text-red-400 mb-2">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={!canStart}
                className="pixel-btn text-[8px] px-3 py-1 border-red-400 text-red-400 hover:bg-red-400/10 disabled:opacity-30"
              >
                CONFIRM START
              </button>
              <button
      ...
      {!isLive && (
        <div className="pixel-panel border-2 border-pixel-border px-3 py-2 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-[8px] text-pixel-gray tracking-wider">CAPITAL (USDC)</span>
            <span className="text-[9px] text-pixel-white font-mono">${liveCapital.toLocaleString()}</span>
          </div>
          <div className="flex items-center gap-1 flex-wrap">
            {CAPITAL_PRESETS.map((p) => (
              <button
                key={p}
                onClick={() => { setLiveCapital(p); setCustomCapital(""); }}
                className={`pixel-btn text-[7px] px-1.5 py-0.5 transition-colors ${
                  liveCapital === p
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setCustomCapital(v);
                const n = Number(v);
                if (n > 0) setLiveCapital(n);
              }}
              placeholder="CUSTOM"
              className="pixel-input-sm w-16 font-mono text-[8px] text-center"
            />
          </div>
        </div>
      )}
      ...
      {!isLive && (
        <div className="pixel-panel border-2 border-pixel-border px-3 py-2">
          <span className="text-[7px] text-pixel-gray tracking-wider block mb-1.5">CHECKLIST</span>
          {[
            ...
            <div key={item.label} className="flex items-center gap-1.5 py-0.5">
              <span className={`text-[8px] ${item.ok ? "text-green-400" : "text-red-400/60"}`}>
                {item.ok ? "[x]" : "[ ]"}
              </span>
              <span className={`text-[8px] font-mono ${item.ok ? "text-pixel-white" : "text-pixel-gray"}`}>
                {item.label}
              </span>
            </div>
          ))}
        ...
        <div className="pixel-panel border-2 border-pixel-border px-3 py-2">
          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
            <div className="flex justify-between">
              <span className="text-[7px] text-pixel-gray">BALANCE</span>
              <span className="text-[8px] text-pixel-white font-mono">
                {engineState.balance !== null ? `$${engineState.balance.toFixed(2)}` : "---"}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-[7px] text-pixel-gray">CAPITAL</span>
              <span className="text-[8px] text-pixel-white font-mono">${liveCapital.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[7px] text-pixel-gray">ORDERS</span>
            ...
            <div className="flex justify-between">
              <span className="text-[7px] text-pixel-gray">VOLUME</span>
              <span className="text-[8px] text-pixel-white font-mono">
                ${engineState.totalVolumeMirrored.toFixed(0)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-[7px] text-pixel-gray">CYCLES</span>
              <span className="text-[8px] text-pixel-white font-mono">{engineState.cycleCount}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[7px] text-pixel-gray">NEXT IN</span>
              <span className="text-[8px] text-green-400 font-mono">{formatCountdown(nextIn)}</span>
            </div>
          </div>
          {engineState.error && (
            <div className="mt-2 px-2 py-1 border border-red-400/40 bg-red-400/5 text-[8px] text-red-400 font-mono">
              {engineState.error}
            </div>
          )}
        </div>
      ...
      {isLive && engineState && engineState.log.length > 0 && (
        <div className="pixel-panel border-2 border-pixel-border">
          <div className="px-3 py-1.5 border-b border-pixel-border flex items-center justify-between">
            <span className="text-[8px] text-pixel-gray tracking-wider">EXECUTION LOG</span>
            <span className="text-[7px] text-pixel-gray font-mono">{engineState.log.length} entries</span>
          </div>
          <div className="max-h-[300px] overflow-y-auto">
            {engineState.log.map((entry) => (
              <div
                key={entry.id}
                className="px-3 py-1 border-b border-pixel-border/20 flex items-start gap-2 text-[8px] font-mono hover:bg-pixel-white/5"
              >
      ...
      {isLive && engineState && engineState.log.length === 0 && (
        <div className="pixel-panel border-2 border-pixel-border px-3 py-4 text-center">
          <span className="text-[9px] text-pixel-gray">WAITING FOR FIRST CYCLE...</span>
        </div>
      )}
    </div>
  );
