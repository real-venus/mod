...
class Mod:
    ...
    def _load_mod_token(self):
        from web3 import Web3
        w3, deploy, account = self._load_deployment()
        abi_path = self.module_dir / 'artifacts' / 'src' / 'contracts' / 'Mod.sol' / 'Mod.json'
        with open(abi_path) as f:
            artifact = json.load(f)
        addr = deploy.get('mod') or deploy.get('subnet')
    ...
    def stake_on(self, validator_key, amount, lock_blocks=0):
        """Stake Mod tokens on a validator (via Staking)."""
        from web3 import Web3
