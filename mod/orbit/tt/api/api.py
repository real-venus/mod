...
ST_ABI_PATH = MODULE_DIR / "artifacts" / "src" / "contracts" / "subnet" / "StakeTime.sol" / "StakeTime.json"
INC_ABI_PATH = MODULE_DIR / "artifacts" / "src" / "contracts" / "subnet" / "Subnet.sol" / "Subnet.json"
NTV_ABI_PATH = MODULE_DIR / "artifacts" / "src" / "contracts" / "NativeToken.sol" / "NativeToken.json"
...
async def register_subnet(req: RegisterSubnetReq):
    ...
    try:
        ...
        result = _send_tx(w3, contract.functions.registerSubnet(
            ...
            Web3.to_checksum_address(req.incentive),
        ), account)
        return {"result": result}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
