...
function StakeTimeApp() {
  ...
  const [subIncentive, setSubIncentive] = useState('')
  ...
  const handleRegisterSubnet = useCallback(async () => {
    if (!subName.trim() || !subStakeTime.trim() || !subIncentive.trim()) {
    ...
    try {
      await api('register_subnet', {
        ...
        incentive: subIncentive.trim(),
      })
      toast.success('Subnet registered')
      setSubName(''); setSubStakeTime(''); setSubIncentive('')
      fetchAll()
    } catch (err: any) {
      toast.error(err?.message || 'Subnet registration failed')
    }
    setRegisteringSub(false)
  }, [subName, subStakeTime, subIncentive, fetchAll])
            ...
            <div className="border border-white/10 rounded-lg p-4 bg-white/[0.02]">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    type="text"
                    placeholder="Incentive contract address..."
                    value={subIncentive}
                    onChange={e => setSubIncentive(e.target.value)}
                    className="flex-1 text-sm px-4 py-2.5 rounded-lg border border-white/10 bg-white/5 text-white/90 focus:outline-none focus:border-white/30 font-mono transition-colors placeholder:text-white/20"
                  />
                  <button
                    onClick={handleRegisterSubnet}
                    disabled={registeringSub || !subName.trim() || !subStakeTime.trim() || !subIncentive.trim()}
