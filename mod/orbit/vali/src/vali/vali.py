...
class Vali:
    ...
    def epoch(self, search=None, result_features=['score', 'name', 'key', 'url', 'duration'], key=None, debug=False, df=True, **kwargs):
        ...
        self.vote(results)
        if df:
            result_df = c.df(results)
            if len(result_df) > 0:
                cols = [f for f in result_features if f in result_df.columns]
                return result_df[cols] if cols else result_df
            return result_df
        return results
    ...
    def vote(self, results):
        ...
        return self.net.vote(
                    modules=[m['key'] for m in results],
                    weights=[m['score'] for m in results],
                    key=self.key, 
                    subnet=self.subnet
                    )
